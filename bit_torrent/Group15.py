...
import asyncio   #asynchronous I/O single-thread concurrent code using non-premptive multitasking
import logging   # for event logging system
import os  #provides OS dependent functionality
import sys  #to access variables used by the interpreter
from contextlib import closing  # used to close socket after work is done
from functools import partial, partialmethod #returns partial object which can freeze some part function 
from math import floor 
from typing import Dict, List, Optional #to simplify complex type signatures
 
import PyQt5
# noinspection PyUnresolvedReferences
import PyQt5.QtCore
from PyQt5.QtCore import Qt, QThread, pyqtSignal,QSize   #for resizing, positions 
import PyQt5.QtGui
...
def load_icon(name: str):
    return QIcon(os.path.join(ICON_DIRECTORY, name + '.jpg'))
...
# 2nd dialog box
    ...
    def _browse(self):
        new_download_dir = QFileDialog.getExistingDirectory(self, 'Select new download directory', self._download_dir)
...
class TorrentListWidgetItem(QWidget):
    ...
    #Form.setStyleSheet("background-image: url();")
    def __init__(self):
        super().__init__()
        vbox = QVBoxLayout(self)
    ...
    def _update(self):
        ...
        if self.waiting_control_action:
            ...
            status_text = 'Download has been  Paused'
        elif state.complete:
...
class MainWindow(QMainWindow):
    def __init__(self, control_thread: 'ControlManagerThread'):
        ...
        toolbar.setContentsMargins(20,20,20,20)
        toolbar.setIconSize(QSize(55,55))
        #toolbar.setGeometry(30,30,0,0)
        ...
        self._pause_action = toolbar.addAction(load_icon('pause'), 'Pause Download')
        self._pause_action.setEnabled(False)
        self._pause_action.triggered.connect(partial(self._control_action_triggered, control.pause))

        self._resume_action = toolbar.addAction(load_icon('resume'), 'Resume Download')
        ...
        self._remove_action = toolbar.addAction(load_icon('remove'), 'Remove Torrent')
        ...
        self.setMinimumSize(x, 730)
        self.resize(10, 10)
        
        self.setWindowTitle('Group 15')
        #self.setStyleSheet("""
         #   #MainWidget {
    ...
    def add_torrent_files(self, paths: List[str]):
        for path in paths:
            try:
                ...
                #if file has already been added  or downloaded to the same location 
...
def main():
    ...
    #if already opened on the same port, then throw error...
    ...
    control_thread = ControlManagerThread()
    #creating  the main pannel for gui
    main_window = MainWindow(control_thread)
    #initialising both task and ui simultaneously
