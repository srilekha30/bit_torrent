...
class SessionStatistics:
    ...
    def downloaded_per_session(self) -> int:
        print("downloaded_per_session", downloaded_per_session)
    ...
    def _get_actual_peer_count(time_dict: Dict[Peer, float]) -> int:
        cur_time = time.time()
        #print("actual peer count", sum(1 for t in time_dict.values() if cur_time - t <= SessionStatistics.PEER_CONSIDERATION_TIME))
    ...
    def downloading_peer_count(self) -> int:
      #  print("Leechers",SessionStatistics._get_actual_peer_count(self._peer_last_download))
        return SessionStatistics._get_actual_peer_count(self._peer_last_download)

    @property
    def uploading_peer_count(self) -> int:
       # print("seeders",SessionStatistics._get_actual_peer_count(self._peer_last_upload))
...
class DownloadInfo:
    ...
    def __init__(self, info_hash: bytes,
        ...
        print("info hash of a piece:", self.info_hash)
...
class TorrentInfo:
    def __init__(self, download_info: DownloadInfo, announce_list: List[List[str]], *, download_dir: str):
        ...
        print(" announce urls:", announce_list)
        self.download_dir = download_dir
        print(" directory into which download is taking place:",download_dir)
        self.paused = False
...
class TorrentState:
    ...
    def __init__(self, torrent_info: TorrentInfo):
        ...
        #print(" leecher", self.downloading_peer_count)
        self.uploading_peer_count = statistics.uploading_peer_count
        #print(" seeder ",self.uploading_peer_count)
